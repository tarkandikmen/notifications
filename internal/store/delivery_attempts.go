...
//     deterministic since dlq_partitions = 1).
//
...
// information ("we have an id but no attempt") if a refinement ever
// wants to.
type UnprocessableInput struct {
	...
	// build the DLQ topic name "send.<channel>.dlq". The arg is
	// authoritative (the Kafka record's topic technically already
	...
	// ErrorCode is one of the documented short codes: "decode_failed",
	// "schema_mismatch", "missing_field", "panic". Written to
	// delivery_attempts.error_message (with ErrorDetails appended for
	// context) on the targeted branch, and to the DLQ payload's "error"
	...
	// DLQPayload is the pre-built JSON body the worker assembled. The
	// store inserts it verbatim into the outbox row's payload column;
	// the relay reads it back and produces it to send.<channel>.dlq.
	DLQPayload json.RawMessage

	// EventPayload is the events.notification payload. Ignored on the
	// no-target branch (statement 4 is skipped). On the targeted branch
	// this is inserted verbatim into the outbox row's payload column.
	EventPayload json.RawMessage

	// TraceHeaders carries W3C Trace Context (JSON) for the outbox rows;
	// zero value stores SQL NULL.
	TraceHeaders json.RawMessage
}

// dlqTopicForChannel renders the per-channel DLQ topic name. The
// channel is trusted here — the worker validated it against the
// validChannels set when binding the kgo consumer in cmd.go — so no
// defensive whitelist runs at the SQL layer.
func dlqTopicForChannel(channel string) string {
	return "send." + channel + ".dlq"
}

// DeliveryAttempt holds the columns of one delivery_attempts table row.
...
// constructs the EventPayload as the events.notification message body;
// RecordOutcome inserts it verbatim into the outbox row.
//
// StartedAt is retained for compatibility but is unused by RecordOutcome:
// Layer 2's BeginAttempt sets started_at in its own transaction, and Tx
// B's first statement is an UPDATE that touches only response /
// classification / error_message / finished_at. The field stays so a
// caller that still passes it doesn't break the build.
...
// BeginAttempt is the worker's Layer 2 idempotency token. Runs the
//
...
// (notification_id, attempt); ack + skip).
//
...
// constraint on this table — so the bare ON CONFLICT DO NOTHING (no
// constraint specified) lands on the right index without naming it.
...
// RecordOutcome runs the worker outcome transaction (Tx B). Three
...
//   - Layer 3 (the attempt-guarded UPDATE, statement 2 here).
...
// RecordUnprocessable runs the T8 transaction. The transaction shape
// branches on whether the worker could extract a (notification_id,
// attempt) target from the corrupt Kafka record:
//
...
//     Wrapped in ON CONFLICT (notification_id, attempt) DO NOTHING so a
//     Kafka redelivery of the same corrupt message is harmless (the
//     second T8 attempt's INSERT would otherwise hit the PK and fail
//     the whole tx, leaving the offset uncommitted forever).
...
//     forensic destination is independent).
...
// No notifications mutation, no delivery_attempts row, no
// events.notification emission. Any orphaned DISPATCHED row eventually
// transitions via the reaper.
//
...
func (s *Store) RecordUnprocessable(ctx context.Context, in UnprocessableInput) error {
	...
	// the Kafka key); null on the no-target branch (Kafka assigns a
	// partition; deterministic at dlq_partitions=1).
